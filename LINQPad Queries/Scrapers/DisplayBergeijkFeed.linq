...
string LOG_FILENAME = @"Podcast\Bergeijk\BergeijkRSSFeed.xml".FromDownloadFolder();
...
if (true)
{
	foreach (var item in items)
	{
		...
		{
			new Hyperlinq (item.Element("enclosure").Attribute("url").Value, title).Dump();	
		}
		else
		{
			"<<Geen Link beschikbaar>>".Dump(); 
		}
